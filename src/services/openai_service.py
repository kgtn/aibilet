    ...
    async def rank_tickets(self, tickets: list) -> dict:
        ...
        try:
            response = await self.client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": """Ранжируй билеты по следующим критериям (в порядке приоритета):
1. Минимальная цена — главный критерий.
2. Минимальное время в пути — важнее, если разница в цене меньше 20%.
3. Наименьшее число пересадок — в случае одинаковой цены и времени в пути.
